...
abstract class ListingDataSourceFactory<T> : DataSource.Factory<Int, T>() {
    val sourceLiveData = MutableLiveData<ListingPageKeyedDataSource<T>>()
    override fun create(): DataSource<Int, T> {
        val source = object : ListingPageKeyedDataSource<T>() {
            private val scope = CoroutineScope(Dispatchers.IO)
            // keep a function reference for the retry event
            private var retry: (() -> Any)? = null
            var refreshLast: (() -> Any)? = null
            ...
            override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, T>) {
                ...
                scope.launch {
                    try {
                        ...
                        refreshLast = { loadAfter(params, callback) }
                    } catch (e: Exception) {
                        retry = {
            ...
            override fun loadInitial(
                ...
                try {
                    ...
                    refreshLast = { loadInitial(params, callback) }
                } catch (e: Exception) {
                    retry = {
            ...
            override fun refreshLast() {
                this.refreshLast?.invoke()
            }
        }
